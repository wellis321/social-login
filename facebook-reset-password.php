...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $valid_token) {
    ...
    if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
        ...
        if (empty($password)) {
            ...
            $stmt = $conn->prepare("UPDATE users SET password_hash = ?, reset_token = NULL, reset_token_expires = NULL WHERE id = ? AND platform = 'facebook'");
            ...
            if ($update_result && $affected_rows > 0) {
                ...
                $verify_stmt = $conn->prepare("SELECT password_hash FROM users WHERE id = ? AND platform = 'facebook'");
