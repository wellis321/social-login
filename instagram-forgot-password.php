...
if (!function_exists('isPhoneNumber') || !function_exists('normalizePhone')) {
    error_log("Missing required functions in instagram-forgot-password.php");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($contact_value)) {
        ...
        try {
            ...
            $result = $conn->query("SELECT id, email FROM users WHERE email = '$input_safe' AND platform = 'instagram'");

            // Check for query errors
            if (!$result) {
                error_log("Database query error in instagram-forgot-password.php: " . $conn->error);
                ...
                if ($result->num_rows === 0 && function_exists('isPhoneNumber') && isPhoneNumber($contact_value)) {
                    ...
                    $all_users = $conn->query("SELECT id, email FROM users WHERE platform = 'instagram'");
                    $found_user = null;
            ...
            error_log("Error in instagram-forgot-password.php: " . $e->getMessage());
