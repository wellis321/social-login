...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $valid_token) {
    ...
    if (empty($password)) {
        ...
        $update_result = $conn->query("UPDATE users SET password_hash = '$password_hash', reset_token = NULL, reset_token_expires = NULL WHERE id = $user_id_safe AND reset_token = '$token_safe' AND platform = 'instagram'");

        if ($update_result) {
            // Log activity
            if (function_exists('logActivity') && $user_id) {
                @logActivity($user_id, 'instagram', 'password_reset', "Password successfully reset for $user_email");
            }
